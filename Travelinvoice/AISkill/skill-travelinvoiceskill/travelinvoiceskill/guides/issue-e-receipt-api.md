...
// --- Test Vectors (Illustrative - Replace with actual production keys/IVs) ---
// AES Key: 32 bytes (256 bits)
// AES IV: 16 bytes (128 bits)
const TEST_AES_KEY = '0123456789abcdef0123456789abcdef'; // Example 32-byte key
const TEST_AES_IV = 'fedcba9876543210';                 // Example 16-byte IV
// SHA HashKey and HashIV: Arbitrary length strings
const TEST_HASH_KEY = 'TestHashKey_For_Signature';
const TEST_HASH_IV = 'TestHashIV_For_Signature';

// --- AES-256-CBC Encryption/Decryption Core Functions ---

/**
 * Applies custom PKCS7 padding with a specified block size.
 * The padding value is equal to the padding length.
 *
 * @param string $plaintext The data to pad.
 * @param int $blockSize The block size for padding (e.g., 32 as per API spec).
 * @return string Padded data.
 */
function custom_pkcs7_pad(string $plaintext, int $blockSize): string
{
    $paddingLength = $blockSize - (strlen($plaintext) % $blockSize);
    // If plaintext is already a multiple of blockSize, pad with blockSize bytes
    if ($paddingLength === 0) {
...
/**
 * Removes custom PKCS7 padding from data.
 *
 * @param string $paddedtext The padded data.
 * @param int $blockSize The block size used for padding (e.g., 32).
 * @return string Unpadded data.
 * @throws Exception If padding is invalid or data is malformed.
 */
function custom_pkcs7_unpad(string $paddedtext, int $blockSize): string
{
    $len = strlen($paddedtext);
    if ($len === 0) {
        throw new Exception("Cannot unpad empty string.");
    }
    if (($len % $blockSize) !== 0) {
        throw new Exception("Decrypted data length is not a multiple of block size. Data might be corrupt.");
    }

    $paddingLength = ord(substr($paddedtext, -1));

    // Padding value must be between 1 and block size
    if ($paddingLength < 1 || $paddingLength > $blockSize) {
        throw new Exception("Invalid padding length detected: " . $paddingLength);
    }

    // Verify all padding bytes have the same value
    for ($i = 1; $i <= $paddingLength; $i++) {
        if (ord(substr($paddedtext, -$i, 1)) !== $paddingLength) {
            throw new Exception("Invalid padding bytes detected.");
        }
    }

    return substr($paddedtext, 0, $len - $paddingLength);
}

/**
 * Encrypts a plaintext string using AES-256-CBC with custom PKCS7 padding (block size 32).
 * Output is in uppercase hexadecimal format.
 *
 * @param string $plaintext The UTF-8 encoded string to encrypt.
 ...
 * @throws Exception On encryption failure or invalid key/IV lengths.
 */
function aes_encrypt_php74(string $plaintext, string $key, string $iv): string
{
    // Validate key and IV lengths
    if (strlen($key) !== 32) {
        throw new Exception("AES encryption key must be 32 bytes.");
    }
    if (strlen($iv) !== 16) {
        throw new Exception("AES encryption IV must be 16 bytes.");
    }

    // Apply custom PKCS7 padding (Block Size 32 as per API spec)
    $paddedPlaintext = custom_pkcs7_pad($plaintext, 32);

    // Encrypt using AES-256-CBC. OPENSSL_RAW_DATA ensures no built-in padding.
    $ciphertext = openssl_encrypt(
        $paddedPlaintext,
        'AES-256-CBC',
        $key,
        OPENSSL_RAW_DATA, // Crucial: Disables built-in padding as we've done it manually
        $iv
    );
    ...
    // Output format: Hexadecimal (uppercase)
    return strtoupper(bin2hex($ciphertext));
}

/**
 * Decrypts a hexadecimal ciphertext string using AES-256-CBC with custom PKCS7 padding (block size 32).
 *
 * @param string $ciphertextHex The ciphertext in hexadecimal format.
 * @param string $key The 32-byte encryption key.
 * @param string $iv The 16-byte initialization vector.
 * @return string The decrypted UTF-8 encoded plaintext.
 * @throws Exception On decryption failure, invalid padding, or invalid key/IV lengths.
 */
function aes_decrypt_php74(string $ciphertextHex, string $key, string $iv): string
{
    // Validate key and IV lengths
    if (strlen($key) !== 32) {
        throw new Exception("AES decryption key must be 32 bytes.");
    }
    if (strlen($iv) !== 16) {
        throw new Exception("AES decryption IV must be 16 bytes.");
    }
    ...
    // Decrypt using AES-256-CBC. OPENSSL_RAW_DATA is used for raw decryption.
    $paddedPlaintext = openssl_decrypt(
        $ciphertext,
        'AES-256-CBC',
        $key,
        OPENSSL_RAW_DATA, // Crucial: For raw decryption to manually unpad
        $iv
    );
    ...
    // Remove custom PKCS7 padding
    return custom_pkcs7_unpad($paddedPlaintext, 32);
}

// --- SHA256 Signature Generation/Verification Core Functions ---

/**
 * Generates an SHA256 signature string based on specified fields, HashKey, and HashIV.
 * The string is formatted as per API specification: HashIV={IV值}&Field1={Value1}&...&HashKey={Key值}.
 * Fields are sorted alphabetically (A-Z). No URL encoding is applied.
 *
 * @param array $fields An associative array of field names and their values.
 ...
 * @return string The SHA256 hash in uppercase hexadecimal format.
 * @throws Exception If required fields for signature are missing.
 */
function sha256_generate_signature_php74(array $fields, string $hashKey, string $hashIv): string
{
    // Fields that participate in the signature, in their specified alphabetical order (A-Z)
    $participatingFields = [
        'InvoiceTransNo',
        'MerchantID',
        'MerchantOrderNo',
    ...
    foreach ($participatingFields as $field) {
        if (!array_key_exists($field, $fields)) {
            throw new Exception("Missing required field for SHA256 signature generation: " . $field);
        }
    }

    // Build the raw string segments
    $segments = [];
    $segments[] = "HashIV={$hashIv}"; // Starts with HashIV

    // Add sorted participating fields
    // Use a temporary array to ensure we only process the *specified* fields and ksort them
    $tempSortedFields = [];
    foreach ($participatingFields as $field) {
        $tempSortedFields[$field] = $fields[$field];
    }
    ksort($tempSortedFields); // Sort by field name (key) A-Z

    foreach ($tempSortedFields as $fieldName => $fieldValue) {
        $segments[] = "{$fieldName}={$fieldValue}";
    }

    $segments[] = "HashKey={$hashKey}"; // Ends with HashKey

    $rawString = implode('&', $segments); // Combine with '&' separator, no URL encoding

    // Compute SHA256 hash and convert to uppercase hexadecimal
    return strtoupper(hash('sha256', $rawString));
}

/**
 * Verifies an SHA256 signature against provided fields, HashKey, and HashIV.
 ...
 * @param array $fields An associative array of field names and their values used for signature generation.
 ...
 */
function sha256_verify_signature_php74(string $receivedSignature, array $fields, string $hashKey, string $hashIv): bool
{
    try {
        $generatedSignature = sha256_generate_signature_php74($fields, $hashKey, $hashIv);
    } catch (Exception $e) {
        // Log the error if signature generation fails due to missing fields, etc.
        // For verification, such failure means the signature cannot be generated, thus it cannot be valid.
        return false;
    }
...
// Example plaintext representing the 'PostData_內含欄位' string
$aesPlaintext = 'Version=1.1&TimeStamp=1700000000&MerchantOrderNo=O_ABC_123&Status=1&CreateStatusTime=&Category=B2C&BuyerName=&BuyerUBN=&BuyerAddress=Taipei&BuyerEmail=test@example.com&Buyerphone=&SellerName=JohnDoe&TotalAmt=1200&EmailLang=0&ItemName=Item1|Item2&ItemCount=1|2&ItemUnit=PC|EA&ItemPrice=1000|100&ItemAmt=1000|200&TourName=TestTour&TourNo=TOUR123&TourDate=2024-12-31&TaxNoted=0&Comment=This is a test comment.';
$aesKey = TEST_AES_KEY;
$aesIv = TEST_AES_IV;

echo "Plaintext (UTF-8):\n" . $aesPlaintext . "\n";
echo "Plaintext Length: " . strlen($aesPlaintext) . " bytes\n";
echo "AES Key (raw): " . $aesKey . "\n";
echo "AES IV (raw): " . $aesIv . "\n";

try {
    $encryptedHex = aes_encrypt_php74($aesPlaintext, $aesKey, $aesIv);
    echo "Encrypted Ciphertext (Hex): " . $encryptedHex . "\n";

    // Expected Ciphertext (for cross-language reference):
    // In a real-world scenario, this would be a pre-calculated, known-good value from the API provider's documentation.
    // For this self-contained demonstration, we use the output of our own function.
    $expectedAesCiphertext = $encryptedHex; // Replace with a constant if provided by API
    echo "Expected Ciphertext (Hex): " . $expectedAesCiphertext . "\n";

    $decryptedPlaintext = aes_decrypt_php74($encryptedHex, $aesKey, $aesIv);
    echo "Decrypted Plaintext (UTF-8):\n" . $decryptedPlaintext . "\n";
    echo "Decryption matches original: " . ($aesPlaintext === $decryptedPlaintext ? "YES" : "NO") . "\n";
...
// Example fields for SHA256 signature (as per API spec)
$shaFields = [
    'InvoiceTransNo' => '193060',
    'MerchantID' => '54352706',
    'MerchantOrderNo' => 'O_201406010001',
...
echo "Participating Fields:\n";
foreach ($shaFields as $key => $value) {
    echo "  {$key}: {$value}\n";
}
...
try {
    // Manually construct the raw string for demonstration visibility
    $rawStringSegments = [];
    $rawStringSegments[] = "HashIV={$hashIv}";
    // Ensure fields are sorted A-Z for building the raw string
    $tempSortForRawString = $shaFields;
    ksort($tempSortForRawString);
    foreach ($tempSortForRawString as $fieldName => $fieldValue) {
        $rawStringSegments[] = "{$fieldName}={$fieldValue}";
    }
    $rawStringSegments[] = "HashKey={$hashKey}";
    $shaRawString = implode('&', $rawStringSegments);

    echo "Raw String (before hashing):\n" . $shaRawString . "\n";

    $generatedSignature = sha256_generate_signature_php74($shaFields, $hashKey, $hashIv);
    echo "Generated Signature (SHA256, Hex, Uppercase): " . $generatedSignature . "\n";

    // Expected Signature (for cross-language reference):
    // In a real-world scenario, this would be a pre-calculated, known-good value from the API provider's documentation.
    // For this self-contained demonstration, we use the output of our own function.
    $expectedShaSignature = $generatedSignature; // Example: 0C69DBB83FD36B6A2B2E9E614DAEA3D91D474C2D8A829870CA91511C55AF2AA
    echo "Expected Signature (SHA256, Hex, Uppercase): " . $expectedShaSignature . "\n";

    // --- Verification Test ---
    $verificationResult = sha256_verify_signature_php74($generatedSignature, $shaFields, $hashKey, $hashIv);
    echo "Signature verification with generated signature: " . ($verificationResult ? "SUCCESS" : "FAILED") . "\n";

    // Test with a tampered signature
    $tamperedSignature = substr($generatedSignature, 0, -1) . ($generatedSignature[strlen($generatedSignature)-1] === 'F' ? '0' : 'F'); // Change last char
    $verificationResultTampered = sha256_verify_signature_php74($tamperedSignature, $shaFields, $hashKey, $hashIv);
    echo "Signature verification with tampered signature: " . ($verificationResultTampered ? "SUCCESS" : "FAILED") . "\n";
